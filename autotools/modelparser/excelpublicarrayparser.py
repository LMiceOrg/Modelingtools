...
Sheet struct: 数组数据类型名称	数据类型描述	数据类型	数组维数
...
class ExcelPublicArrayParser(excelsheetparser.ExcelSheetParser):
    def ParseExcelSheet(self, model, xl_name, sh_ctx, sh_idx, sh_name):
        ...
        for i in range(1, len(sh_ctx)):
            if len(sh_ctx[i]) < 4:
                continue
            ad_name, ad_desc, ad_type, ad_dim = sh_ctx[i][:4]
            #ignore empty row
            if ad_name == "":
                continue
            #print "array:",self.ns,ad_name
            model.AppendItem(xl_name, sh_idx, sh_name, self.ns, "ArrayData", (ad_name, ad_desc, ad_type, ad_dim))
